...
impl Auth for AuthService {
    /// If the provided `username` and `password` are valid, create a `session_token` for this user.
    ...
    async fn sign_in(
        ...
        let response = match sign_in_result {
            None => SignInResponse {
            ...
            Some(user_uuid) => {
                // Sign in success. Create a session for this user.
    ...
    /// Register a new user with this `username` and `password` if valid.
    /// Note: This operation is SLOW due to password hashing.
    async fn sign_up(
        &self,
        ...
        println!("  Got sign_up_result");

        let response = match sign_up_result {
            Ok(_) => SignUpResponse {
                status_code: StatusCode::Success.into(),
            },
